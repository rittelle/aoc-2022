...
(defn read-ls-output
  "Return a sequence of pairs [size path] from an ls output."
...
(defn flatmap-dirs
  "Apply f to all directories and return a flattened sequence."
  [f tree]
  (if (number? tree)
    []
    (concat [(f tree)]
            (flatten (map (partial flatmap-dirs f) (vals tree))))))
                  ...
                  (let [current-line (first lines-left)
                    ...
                    (case command
                      "ls" (let [[ls-output remaining-session] (split-with #(not (string/starts-with? % "$ ")) remaining-lines)
                                 new-files (map (partial read-ls-output current-path) ls-output)]
                             (recur remaining-session current-path (reduce add-entry-to-tree tree new-files)))
                      "cd" (recur remaining-lines (read-path-in-context current-path (nth words 2)) tree)
                      (recur remaining-lines current-path tree))))))

(defn solution-1
  "Solve the first part"
  [input-str] (->> input-str
                   (read-input-1)
                   (flatmap-dirs calculate-total-size)
...
(defn solution-2
  ...
  [input-str] (let [total-space 70000000
                    ...
                    sizes (flatmap-dirs calculate-total-size tree)
                    ]
                ;; there will be no bigger size than total-space
                (transduce (filter (partial < needed-space)) min total-space sizes)))
